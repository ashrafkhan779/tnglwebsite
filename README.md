...
- **Dark / light theme** — a toggle in the nav switches the whole site between the light orange-and-white theme and a premium dark theme, with the choice remembered between visits.
- **Cinematic page transitions** — a staggered column wipe plays on load and when moving between pages, giving the multi-page site an app-like, single-flow feel.
- **Bigger, scroll-reactive 3D globe** — the Global Network globe is larger and more immersive; it orbits as you scroll, spins on drag, and pauses automatically when off-screen to save battery.
- **Premium "futuristic tech" pass** — depth and motion throughout: hero **particle constellation** + tech-grid + parallax, **glassmorphism**, **3D tilt** cards, glow orbs, animated gradient border, cursor spotlight, button sheen, and a scroll-progress bar.
